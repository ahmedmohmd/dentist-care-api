...
const getAllDates: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (allDatesFromCache) {
      return customResponseUtil.successResponse(res, HttpCode.OK, allDatesFromCache)
    }
...
const releaseAllDates: RequestHandler = async (req, res, next) => {
  try {
    ...
    return customResponseUtil.successResponse(res, HttpCode.NO_CONTENT, 'All Dates have been released successfully')
  } catch (error) {
    next(error)
  }
}
