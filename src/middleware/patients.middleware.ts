...
const validatePatientExistance: RequestHandler = async (req, res, next) => {
  ...
  if (!targetPatient) {
    return customResponseUtil.errorResponse(res, HttpCode.NOT_FOUND, 'Patient not found')
  }
...
const validatePatientIdParam: RequestHandler = async (req, res, next) => {
  ...
  if (!patientId) {
    return customResponseUtil.errorResponse(res, HttpCode.BAD_REQUEST, 'Invalid Patient ID parameter')
  }
...
const validateCreatePatient: RequestHandler = async (req, res, next) => {
  const validatorResult: any = patientsValidator.CreatePatient.safeParse(req.body)

  if (!validatorResult.success) {
    const errorMessage = validatorResult.error.errors.map((error: any) => error.message).join('; ')
    return customResponseUtil.errorResponse(res, HttpCode.BAD_REQUEST, `Patient is not valid: [${errorMessage}]`)
  }

  const targetPatient = await patientsService.getPatientByEmail(req.body.email)

  if (targetPatient) {
    return customResponseUtil.errorResponse(res, HttpCode.BAD_REQUEST, 'Patient already exists')
  }
...
const validateUpdatePatient: RequestHandler = async (req, res, next) => {
  const validatorResult: any = patientsValidator.UpdatePatient.safeParse(req.body)
