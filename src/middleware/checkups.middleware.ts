...
const validateCreateCheckup: RequestHandler = async (req, res, next) => {
  ...
  const targetCheckup = await checkupsService.getCheckupByPatientId(
    req.body.patientId
  );
  ...
  if (targetCheckup[0]) {
    return customResponseUtil.errorResponse(
      res,
      HttpCode.BAD_REQUEST,
      "Checkup already exists for this patient"
    );
  }
...
const validateUpdateCheckup: RequestHandler = async (req, res, next) => {
  const validatorResult: any = checkupsValidator.UpdateCheckup.safeParse(
    req.body
  );
